// Example plugin entry (illustrative — this lives in docs, not in the build).
//
// The host extracts the archive, validates manifest.json, then imports this module
// and calls activate(api). `api` is a permission-scoped WorkbenchAPI: it always
// carries the UI model classes (Activity, PanelView, StatusView, …) plus the
// facade slices the manifest's `permissions` grant — here "activities",
// "commands", and "keybindings". There is no api.menus / api.modals because the
// manifest did not request them.
//
...
// self-contained; a real plugin would import .vue/.jsx single-file components.
const HelloPanel = {
