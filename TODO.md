...
- [ ] Electron first paint: kill the blank white screen shown before the app layout appears. Recommended layers (no SSR — prerender+hydration adds real complexity for a local SPA and a static skeleton achieves the same perceived result):
  1. In `client/electron/main.js`: create the window with `show: false` and a `backgroundColor` matching the theme background, then `win.once('ready-to-show', () => win.show())` — the window only appears after the renderer's first paint, and anything residual is theme-colored instead of white.
  2. Inline boot splash in `client/index.html`: a static CSS-only skeleton of the chrome (title-bar strip, activity-bar rail, sidebar/editor panel blocks) or a centered logo, with critical CSS inlined in the HTML so it paints before any JS. Persist the active theme's background color (e.g. `localStorage['fw:theme-bg']`, written whenever the theme changes) and read it in an inline `<script>` so dark themes never flash light. The Vue app replaces the skeleton on mount; optionally keep it as an opaque overlay until workbench layout restore completes, then fade it out (~150 ms) so the layout never visibly pops in.
  3. (Related to the background-preload idea above: with `show:false` + `ready-to-show` the window can also be pre-created and kept hidden for instant reopen.)
