...
- **Backend**: the Go HTTP server lives in the **`@files-workbench/core`** package (sibling checkout `../files-workbench-core`, `server/` dir) — data server on port 8001, control server on port 8002, all routes under `/_api/v1/`. The same package's `src/` is the JS client library (`fs-api`, `explorer-api`, `sw-queue`, `plugin-rpc`, `plugins-api`, `api-config`, …) imported as `'@files-workbench/core'`. `npm run dev:server` runs the server from the package with `FW_*` env vars pointing at THIS repo's `config/` and `.fw/plugins` (the packaged app does the same from Electron's main.js).
- **No dev proxy**: the client calls both servers directly with absolute URLs from `@files-workbench/core`'s `api-config` (`API_BASE` → 8001 reads, `CONTROL_BASE` → 8002 writes; CORS is permissive on the Go side). Override with `VITE_API_BASE` / `VITE_CONTROL_BASE` in `client/.env`.
...
The workbench is organized into **activities** — self-contained feature modules. Only the core `workbench` shell is in-core in `client/activities/`; `explorer` is the one core-bundled plugin (`client/builtin-plugins/`), and `preview`, `details`, `debug`, `source-control`, `material-icon-theme`, `chat`, `search`, `storage`, and `converter` are first-party **runtime plugins** built from the top-level `/plugins` tree — they are still activities, just registered through the plugin host rather than compiled into `ACTIVITIES`. (`search` is a working file-search panel over the filesystem index — see below; `chat`/`storage`/`converter` are still placeholder panels awaiting their roadmap build-out.) Each declares the surfaces it contributes and, optionally, a runtime **API** for collaboration. This is the foundation of the plugin system (see **Plugin system** below) — first-party activities use the same internal API a plugin does, narrowed by permission.
...
- **panel views + sections** → unchanged; `ViewContentHost.vue` + the SplitView system render them. The framework's view registry just sources them from activities now.
...
- **Manifest + permissions** (`@workbench/framework` models): the Chrome-style `manifest.json` validator and the permission catalog (front-end `PERMISSIONS` gating facade slices, `HOST_PERMISSIONS` reserved). JSON schema + example: the `@workbench/plugin-sdk` package (`schema/manifest.schema.json`, `example/`).
- **SDK** (`@workbench/plugin-sdk` + `client/sdk.js`): the package owns the *mechanism* — `installFwSdk` publishes `globalThis.__FW_SDK`, the plugin build externalizes `vue`/`@workbench/plugin-sdk` imports to it (legacy alias `@fw/sdk`), plus the capability scan, prototype hardening, the WASM `ServerPlugin` SDK, and the esbuild helpers. The *surface* plugins receive is host policy, composed in `client/sdk.js` (UI models, safe read-only fs helpers, shared components like the lazy MonacoEditor) — grow it deliberately; privileged ops stay behind the permission-gated `api`.
- **Loader** (`@workbench/framework`): `createPluginApi` builds the frozen, permission-scoped `api` (UI classes + `log` + only the granted facade slices; the `server` slice's transport is the app-supplied `services.callPluginRpc` from `@files-workbench/core`); `createPluginHost` loads/unloads `{ manifest, module }` pairs (dependency-ordered, lifecycle). Runtime *delivery* (fetch, hash-verify, blob-import) stays in the app: `client/composables/plugins/useRuntimePlugins.js`. `activate(api)` may be sync or async and returns (or resolves to) a disposer; `deactivate(api)` is optional. **Fault isolation**: the host imports optional plugins lazily and in parallel, then activates them sequentially — an import failure, a thrown/rejected `activate()`, or a bogus disposer is logged and confined to that plugin (peers and the host keep running); a reactive `states` map tracks each plugin's `loading`/`active`/`failed` status.
...
- **Sandboxed backend**: plugins never touch the filesystem/control server directly — a plugin ships its own WASM backend (`server` block + `server` permission) that the Go host runs sandboxed (core `server/plugin_host.go`, extism/wazero) and the client reaches via `api.server.call(...)` → `POST /_api/v1/plugins/{id}/rpc`. Source Control's git backend is the reference (it replaced the former `scm.go` handlers and `api.scm` broker).
...
| File | Purpose |
|---|---|
| *(moved)* | The API client library (`api-config`, `fs-api`, `explorer-api`, `explorer-roots`, `sw-queue`, `plugin-rpc`, `plugins-api`, `perf-log`) moved into **`@files-workbench/core`** — import from the package (same export names). `capability-scan` moved into **`@workbench/plugin-sdk`**; `client/lib/` is gone. |
...
## Go server handlers (`../files-workbench-core/server/`)
