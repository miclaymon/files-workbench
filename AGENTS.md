...
| File | Purpose |
|---|---|
| *(moved)* | The API client library (`api-config`, `fs-api`, `explorer-api`, `explorer-roots`, `sw-queue`, `plugin-rpc`, `plugins-api`, `perf-log`) moved into **`@files-workbench/core`** — import from the package (same export names). `client/lib/` now holds only `capability-scan.mjs` (plugin build/consent tooling; SDK-bound in M5). |
