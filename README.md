...
- Fast file search: a background index service ([`@files-workbench/core`](../files-workbench-core) → `fw-indexer`, SQLite FTS5) with instant substring name/path search, live-updating results (create/modify/delete reflected without a rescan), and click-to-reveal; the **Search** panel drives it (content full-text search is on the roadmap)
